...
public class MockProxy implements IProxy
{
	...
	private void readGameFile()
	{
		 ...
		 * proxy.postGameCommands(commandParam); proxy.saveGame(new
		 * SaveGameParam(game.getGameId(),"junit")); games[0] = proxyGame;
		 */
	}
	...
	{
		//
		// if(input.getUser() != null || input.getPassword() != null ||
		// input.getUser() == "hank")
		// {
		//
		// return new StandardResponse(false);
	...
	public CreateGameResponse createGame(CreateGameParam input)
	{
		...
		PlayerListObject[] players = fakeGame.getPlayerListObject();
		CreateGameResponse result = new CreateGameResponse(title, id, players,
				b);
	...
	public GameModelResponse finishTurn(FinishTurnParam input)
	{
		...
		GameModelResponse result = new GameModelResponse();
		fakeGame.getTurnTracker().setCurrentTurn(
				fakeGame.getTurnTracker().getCurrentTurn() + 1);
	...
	public GameModelResponse playYearOfPlenty(PlayYearOfPlentyParam input)
	{
		...
		currentPlayer.getOldDevCards().setYearOfPlenty(
				currentPlayer.getOldDevCards().getYearOfPlenty() - 1);

		currentPlayer.getResources().addResource(ResourceType.WOOD, 1);
		currentPlayer.getResources().addResource(ResourceType.BRICK, 1);

		// System.out.println("proxy wood is" +
		// currentPlayer.getResources().getResourceAmount(ResourceType.WOOD));
	...
	public GameModelResponse offerTrade(OfferTradeParam input)
	{
		...
		TradeOffer offer = new TradeOffer(0, input.getreceiver(),
				input.getOffer());

		fakeGame.setTradeOffer(offer);
		fakeGame.getPlayers()[0].getResources().useResource(ResourceType.WOOD,
				1);
		fakeGame.getPlayers()[0].getResources().addResource(ResourceType.SHEEP,
				1);
		result.setGame(fakeGame);
		result.setValid(true);
	...
	public GameModelResponse maritimeTrade(MaritimeTradeParam input)
	{
		...
		fakeGame.getPlayers()[input.getPlayerIndex()].getResources()
				.useResource(ResourceType.WOOD, 4);
		fakeGame.getPlayers()[input.getPlayerIndex()].getResources()
