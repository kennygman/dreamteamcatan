...
public class ModelFacade extends Observable implements IModelFacade
{
	...
	{
		if (game == null || game.getTurnTracker() == null || player == null
				|| player.getPlayerIndex() > 3)
			return false;
		return game.getTurnTracker().getCurrentTurn() == player
				.getPlayerIndex();
	}
	...
	private boolean canPlay()
	{
		...
		if (getState().equals(TurnTracker.FIRSTROUND)
				...
				|| getState().equals(TurnTracker.ROBBING))
			return true;
		return false;
	}
	...
	public boolean CanRollNumber()
	{
		boolean result = (this.isPlayerTurn()
				&& game.getTurnTracker().getStatus()
						.equals(TurnTracker.ROLLING) && !hasRolled);
		if (result)
		{
			hasRolled = true;
		}
			...
			String outResource)
	{
		if (!canPlay())
			return false;
		if (getPlayer().getResources().getResourceAmount(
				ResourceType.fromString(inputResource)) < ratio)
			return false;
	...
	public boolean CanUseYearOfPlenty(String resource1, String resource2)
	{
		if (!canPlayDevCard(DevCardType.YEAR_OF_PLENTY))
			return false;
		if (game.getBank()
				.getResourceAmount(ResourceType.fromString(resource1)) > 0
				&& game.getBank().getResourceAmount(
						ResourceType.fromString(resource2)) > 0)
			return true;
		return false;
	}
	...
	public boolean CanUseRoadBuilder(EdgeLocation spot1, EdgeLocation spot2)
	{
		...
		if (game.getBoard().hasNeighborRoad(spot2,
				getPlayer().getPlayerIndex(), getState()))
		{
			return true;
		}
	...
	public boolean CanUseMonument()
	{
		if (!isPlayerTurn())
			return false;
		else if (!getState().equals(TurnTracker.PLAYING))
			return false;
		else if (!getPlayer().getOldDevCards().hasDevCard(DevCardType.MONUMENT))
			return false;
		else if (getPlayer().isPlayedDevCard())
			return false;
		return true;
	}
	...
	public void buildSettlement(VertexLocation vert, boolean free)
	{
		GameModelResponse response = proxy
				.buildSettlement(new BuildSettlementParam(player
	...
	{
		GameModelResponse response = proxy.buildCity(new BuildCityParam(player
				.getPlayerIndex(), vert));
		if (response.isValid())
		{
			game = response.getGame();
	...
	{
		GameModelResponse response = proxy
				.maritimeTrade(new MaritimeTradeParam(player.getPlayerIndex(),
	...
	{
		GameModelResponse response = proxy.robPlayer(new RobPlayerParam(player
				.getPlayerIndex(), victimIndex, location));
	...
	{
		GameModelResponse response = proxy.finishTurn(new FinishTurnParam(
	...
	{
		GameModelResponse response = proxy.buyDevCard(new BuyDevCardParam(
				player.getPlayerIndex()));
		if (response.isValid())
		{
			game = response.getGame();
	...
	{
		GameModelResponse response = proxy
				.playYearOfPlenty(new PlayYearOfPlentyParam(player
	...
	{
		GameModelResponse response = proxy
				.playRoadBuilding(new PlayRoadBuildingParam(player
	...
	public StandardResponse joinGame(JoinGameParam params)
	{
		...
		if (response.isValid())
		{
			this.player.setColor(CatanColor.fromString(params.getColor()));
			this.hasJoined = true;
		}
		return response;
	}
	...
	public PlayerInfo[] getPlayerInfoList()
	{
		...
		List<PlayerInfo> playerList = response.getGameListObject(
				gameInfo.getId()).getPlayers();
		players = playerList.toArray(new PlayerInfo[0]);
		return players;
