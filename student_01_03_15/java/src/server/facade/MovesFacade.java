...
public class MovesFacade implements IMovesFacade
{
	...
	public GameModelResponse buildRoad(BuildRoadParam param, int id)
	{
		...
		if (ModelFacade.getInstance().canPlaceRoad(param.getRoadLocation(),
				param.isFree()))
		{
			valid = true;
			cmd.execute();
	...
	public GameModelResponse buildSettlement(BuildSettlementParam param, int id)
	{
		...
		if (ModelFacade.getInstance().canPlaceSettlement(param.getLocation(),
				param.isFree()))
		{
			valid = true;
			cmd.execute();
	...
	public GameModelResponse maritimeTrade(MaritimeTradeParam param, int id)
	{
		...
		if (ModelFacade.getInstance().CanMaritimeTrade(param.getRatio(),
				param.getInputResource(), param.getOutResource()))
		{
			valid = true;
			cmd.execute();
			// games.addCommand(id, cmd);
	...
	public GameModelResponse robPlayer(RobPlayerParam param, int id)
	{
		...
		if (ModelFacade.getInstance().canRobPlayer(param.getLocation(),
				param.getVictimIndex()))
		{
			valid = true;
			cmd.execute();
			// games.addCommand(id, cmd);
	...
	public GameModelResponse playSoldierCard(PlaySoldierParam param, int id)
	{
		...
		if (ModelFacade.getInstance().CanUseSoldier(param.getVictimIndex(),
				param.getLocation()))
		{
			valid = true;
			cmd.execute();
			...
			int id)
	{
		...
		if (ModelFacade.getInstance().CanUseYearOfPlenty(param.getResource1(),
				param.getResource2()))
		{
			valid = true;
			cmd.execute();
	...
	public GameModelResponse playRoadCard(PlayRoadBuildingParam param, int id)
	{
		...
		if (ModelFacade.getInstance().CanUseRoadBuilder(param.getSpot1(),
				param.getSpot2()))
		{
			valid = true;
			cmd.execute();
