...
public class ModelFacadeUnitTest
{
	...
	// This is testing to make sure that the Can Build road won't let me build
	// when I shouldn't
	@Test
	...
	public void testCanBuildSettlement()
	{
		...
		// build road to connecting settlement
		...
		if (ModelFacade.getInstance().canPlaceSettlement(
				settlement.getLocation(), false))
		{
	...
	public void testCantBuildSettlement()
	{
		...
		boolean result = ModelFacade.getInstance().canPlaceSettlement(
				settlement.getLocation(), false);
		assertEquals(result, false);
	}
	...
	public void testCanDisCardCards()
	{
		...
		Player player = game.getPlayer(ModelFacade.getInstance()
				.getPlayerInfo().getPlayerIndex());
	...
	public void testCanUseYearOfPlenty()
	{
		...
		if (ModelFacade.getInstance().CanUseYearOfPlenty("wood", "brick"))
		{
			...
			{
				System.out
						.println("post wood is" + postWood + "wood is" + wood);
				result = false;
			}
			if (postBrick != brick + 1)
			{
	...
	public void testCanUseRoadBuilding()
	{
		...
		{
			// Testing to see if road is taken these if statements should return
			// false bc roads have been built
